...
@tool(description="获取指定城市的天气，以消息字符串的姓氏返回")
...
def fetch_external_data(user_id:str,month:str) -> str:
    ...
    try:
        ...
        logger.warning(f"[fetch_external_data]为检索到用户:{user_id}在{month}的使用记录！")
        return ""

@tool(description="无入参，无返回值。调用后触发中间件，自动为报告生成的场景注入上下文信息，为后续提示词切换提供上下文信息。")
